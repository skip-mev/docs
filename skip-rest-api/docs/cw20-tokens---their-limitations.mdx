...
This page covers the basics of CW20s and the limitations around performing cross-chain actions with CW20 tokens -- compared to tokenfactory and "native Cosmos" assets. 

### CW20 Token Denom Formatting In API Requests

To use CW20 tokens in the Skip API, specify the denom as "cw20:" + the token contract address. 

Example denom for Astro on Terra: cw20:terra1nsuqsk6kh58ulczatwev87ttq2z6r3pusulg9r24mfj2fvtzd4uq3exn26
...
* Sending tokens to a contract along with a message
* Tracking balances
...
* Like CW20s, they're permissionless and users can create new ones just by submitting transactions -- no need to modify the chain itself
...
This combination of traits leads many to see tokenfactory as a strict improvement on CW20 that devs should prefer in the vast majority of cases.

### What limitations do CW20 tokens have within the Skip API?
...
![Chain 1 is the origin chain, where the token can be used freely for post-route actions, but it cannot be used in post-route actions on other chains.](/images/568550d-case2.jpg)
...
![Chain 2 is the origin chain. The token can be transferred back there, but it can't be used or swapped for anything in the same transaction.](/images/3dde446-case3.jpg)
...
In principle, you can use the Skip API to construct any of these action sequences across multiple transactions, but it will be more challenging for you and your end users.
