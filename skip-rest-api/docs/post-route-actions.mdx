...
Callers of the Skip API can specify actions to execute on the destination chain after a route transfer or swap is complete using the `post_route_handler` parameter of `/v2/fungible/msgs`as a part of the same transaction where the swap/transfer took place. 
...
To specify a CosmWasm contract call on the destination chain, pass a `wasm_msg` as the `post_route_handler` in the `/v1/fungible/msgs` call with: 
...
To specify an Autopilot action on the destination chain, pass a `autopilot_msg` as the `post_route_handler` in the `/v1/fungible/msgs` call with: 
