...
Simply follow the instructions from the quickstart guide about [how to compile your node with `mev-tendermint`](select/validator/0-quickstart.md#2-compile-your-node-with-mev-tendermint) each time.
...
Because the Injective codebase is not open source, injective requires a different process. You must follow the special Injective instructions [here](select/validator/8-injective-build.md). (Don't worrry, they're simple.)
:::
