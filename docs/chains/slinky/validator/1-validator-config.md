...
The main oracle configuration object is located in [oracle.go](https://github.com/skip-mev/slinky/blob/main/oracle/config/oracle.go). This is utilized to set up the oracle, whether in process or out of process, and to configure the providers that the oracle will use. The object is defined as follows:
...
This flag is utilized to note whether the oracle side car is running in process or out of process. If this flag is set to true, then the oracle side car will be run in process, and the remaining fields will be used to configure the oracle. If this flag is set to false, then the `Providers`, `CurrencyPairs`, and `UpdateInterval` fields will be unused as these _must_ be configured on the out-of-process oracle that is running.
...
This field is utilized to set the timeout for the oracle client. This is the time that the oracle client will wait for a response from the oracle server before timing out i.e. how long will [`ExtendVote`](https://github.com/skip-mev/slinky/blob/main/abci/ve/vote_extension.go) wait for a response from the oracle server before timing out.
...
This field is utilized to set the list of providers that the oracle will fetch prices from.
...
There two types of oracle instrumentations that can be configured: **app-side oracle metrics** and **oracle side-car metrics**. The metrics configuration can be found in [metrics.go](https://github.com/skip-mev/slinky/blob/main/oracle/config/metrics.go). The metrics configuration object is defined as follows:
